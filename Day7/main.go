...
type UpdateNoteInput struct {
	Title *string `json:"title" binding:"omitnil,max=100"`
	Content *string `json:"content" binding:"omitnil,min=1"`
}
var db *pgxpool.Pool
...
func patchNote(c *gin.Context) {
	...
	if input.Content != nil {
		query += "content = $"+ strconv.Itoa(argIdx) + ", "
		args = append(args, *input.Content)
		argIdx++
	}

	query = query[:len(query) - 2] + " WHERE id = $" + strconv.Itoa(argIdx) + " RETURNING id, title, content, created_at"
	args = append(args, id)
	...
	if err != nil {
		c.JSON(http.StatusNotFound, gin.H{"error": "The note with this id doesnt exist"})
		return
	}
