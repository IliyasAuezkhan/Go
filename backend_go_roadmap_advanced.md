...
- Слоистая архитектура: `handler → service → repository` Done
- Dependency Injection без магии (руками, через интерфейсы) Done
- Разделение на пакеты: `internal/`, `pkg/`, `cmd/` Done
- Domain-driven подход: сущности, DTO, мапперы Done
